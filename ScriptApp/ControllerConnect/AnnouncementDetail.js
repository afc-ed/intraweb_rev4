...
app.controller('Connect/AnnouncementDetail', function ($scope, appFactory, $modalInstance, $modal, header)
{
    window.scope = $scope;    
    $scope.input = header;
    Spinner($scope, 'off');
    ...
    {
        appFactory.postRequest('/Connect/AnnouncementDetailGetData', $scope.input)
    ...
    $scope.save = function ()
    {
        ...
        {
            Spinner($scope, 'on'); 
            appFactory.postRequest('/Connect/AnnouncementSave', $scope.input)
                .then(function (response)
                {
                    if (!appFactory.errorCheck(response))
                    {  
    ...
    $scope.delete = function () {
        if (ConfirmDelete()) {
            Spinner($scope, 'on'); 
            appFactory.postRequest('/Connect/AnnouncementDelete', $scope.input)
                .then(function (response) {
                    if (!appFactory.errorCheck(response)) {
                        $modalInstance.close();
                    }
        ...
        var modalInstance = $modal.open(
            {
                ...
                resolve:
                {
                    header: function () {
                        ...
                        $scope.input.Parent = "announcement";
                        return $scope.input;
                    }
                }
            });
